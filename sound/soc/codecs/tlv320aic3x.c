...
static int aic3x_init(struct snd_soc_codec *codec)
{
	...
	ret=snd_soc_write(codec, AIC3X_PAGE_SELECT, PAGE0_SELECT);
	if(ret<0)
		return ret;
	ret=snd_soc_write(codec, AIC3X_RESET, SOFT_RESET);
	if(ret<0)
		return ret;
	/* DAC default volume and mute */
	ret=snd_soc_write(codec, LDAC_VOL, 0x00);
	if(ret<0)
		return ret;
	ret=snd_soc_write(codec, RDAC_VOL, 0x00);
	if(ret<0)
		return ret;

	/* DAC to HP default volume and route to Output mixer */
	ret=snd_soc_write(codec, DACL1_2_HPLOUT_VOL, ROUTE_ON);
	if(ret<0)
		return ret;
	ret=snd_soc_write(codec, DACR1_2_HPROUT_VOL, ROUTE_ON);
	if(ret<0)
		return ret;
	ret=snd_soc_write(codec, DACL1_2_HPLCOM_VOL, ROUTE_ON);
	if(ret<0)
		return ret;
	ret=snd_soc_write(codec, DACR1_2_HPRCOM_VOL, ROUTE_ON);
	if(ret<0)
		return ret;

	/* DAC to Line Out default volume and route to Output mixer */
	ret=snd_soc_write(codec, DACL1_2_LLOPM_VOL, ROUTE_ON);
	if(ret<0)
		return ret;
	ret=snd_soc_write(codec, DACR1_2_RLOPM_VOL, ROUTE_ON);
	if(ret<0)
		return ret;

	/* DAC to Mono Line Out default volume and route to Output mixer */
	ret=snd_soc_write(codec, DACL1_2_MONOLOPM_VOL, DEFAULT_VOL | ROUTE_ON);
	if(ret<0)
		return ret;
	ret=snd_soc_write(codec, DACR1_2_MONOLOPM_VOL, DEFAULT_VOL | ROUTE_ON);
	if(ret<0)
		return ret;

	/* unmute all outputs */
	reg = snd_soc_read(codec, LLOPM_CTRL);
	ret=snd_soc_write(codec, LLOPM_CTRL, reg | UNMUTE);
	if(ret<0)
		return ret;
	reg = snd_soc_read(codec, RLOPM_CTRL);
	ret=snd_soc_write(codec, RLOPM_CTRL, reg | UNMUTE);
	if(ret<0)
		return ret;
	reg = snd_soc_read(codec, MONOLOPM_CTRL);
	ret=snd_soc_write(codec, MONOLOPM_CTRL, reg | UNMUTE);
	if(ret<0)
		return ret;
	reg = snd_soc_read(codec, HPLOUT_CTRL);
	ret=snd_soc_write(codec, HPLOUT_CTRL, reg | UNMUTE);
	if(ret<0)
		return ret;
	reg = snd_soc_read(codec, HPROUT_CTRL);
	ret=snd_soc_write(codec, HPROUT_CTRL, reg | UNMUTE);
	if(ret<0)
		return ret;
	reg = snd_soc_read(codec, HPLCOM_CTRL);
	ret=snd_soc_write(codec, HPLCOM_CTRL, reg | UNMUTE);
	if(ret<0)
		return ret;
	reg = snd_soc_read(codec, HPRCOM_CTRL);
	ret=snd_soc_write(codec, HPRCOM_CTRL, reg | UNMUTE);
	if(ret<0)
		return ret;

	/* ADC default volume and unmute */
	ret=snd_soc_write(codec, LADC_VOL, 0x06);
	if(ret<0)
		return ret;
	ret=snd_soc_write(codec, RADC_VOL, 0x06);
	if(ret<0)
		return ret;

	/* By default route Line to ADC PGA mixer */
	ret=snd_soc_write(codec, LINE1L_2_LADC_CTRL, 0x0);
	if(ret<0)
		return ret;
	ret=snd_soc_write(codec, LINE1R_2_RADC_CTRL, 0x0);
	if(ret<0)
		return ret;
	ret=snd_soc_write(codec, LINE2L_2_LADC_CTRL, 0x0);
	if(ret<0)
		return ret;
	ret=snd_soc_write(codec, LINE2R_2_RADC_CTRL, 0x0);
	if(ret<0)
		return ret;

	/* PGA to HP Bypass default volume, disconnect from Output Mixer */
	ret=snd_soc_write(codec, PGAL_2_HPLOUT_VOL, DEFAULT_VOL);
	if(ret<0)
		return ret;
	ret=snd_soc_write(codec, PGAR_2_HPROUT_VOL, DEFAULT_VOL);
	if(ret<0)
		return ret;
	ret=snd_soc_write(codec, PGAL_2_HPLCOM_VOL, DEFAULT_VOL);
	if(ret<0)
		return ret;
	ret=snd_soc_write(codec, PGAR_2_HPRCOM_VOL, DEFAULT_VOL);
	if(ret<0)
		return ret;
	/* PGA to Line Out default volume, disconnect from Output Mixer */
	ret=snd_soc_write(codec, PGAL_2_LLOPM_VOL, DEFAULT_VOL);
	if(ret<0)
		return ret;
	ret=snd_soc_write(codec, PGAR_2_RLOPM_VOL, DEFAULT_VOL);
	if(ret<0)
		return ret;
	/* PGA to Mono Line Out default volume, disconnect from Output Mixer */
	ret=snd_soc_write(codec, PGAL_2_MONOLOPM_VOL, DEFAULT_VOL);
	if(ret<0)
		return ret;
	ret=snd_soc_write(codec, PGAR_2_MONOLOPM_VOL, DEFAULT_VOL);
	if(ret<0)
		return ret;

	/* Line2 to HP Bypass default volume, disconnect from Output Mixer */
	ret=snd_soc_write(codec, LINE2L_2_HPLOUT_VOL, DEFAULT_VOL);
	if(ret<0)
		return ret;
	ret=snd_soc_write(codec, LINE2R_2_HPROUT_VOL, DEFAULT_VOL);
	if(ret<0)
		return ret;
	ret=snd_soc_write(codec, LINE2L_2_HPLCOM_VOL, DEFAULT_VOL);
	if(ret<0)
		return ret;
	ret=snd_soc_write(codec, LINE2R_2_HPRCOM_VOL, DEFAULT_VOL);
	if(ret<0)
		return ret;
	/* Line2 Line Out default volume, disconnect from Output Mixer */
	ret=snd_soc_write(codec, LINE2L_2_LLOPM_VOL, DEFAULT_VOL);
	if(ret<0)
		return ret;
	ret=snd_soc_write(codec, LINE2R_2_RLOPM_VOL, DEFAULT_VOL);
	if(ret<0)
		return ret;
	/* Line2 to Mono Out default volume, disconnect from Output Mixer */
	ret=snd_soc_write(codec, LINE2L_2_MONOLOPM_VOL, ROUTE_ON);
	if(ret<0)
		return ret;
	ret=snd_soc_write(codec, LINE2R_2_MONOLOPM_VOL, ROUTE_ON);
	if(ret<0)
		return ret;

	if (aic3x->model == AIC3X_MODEL_3007) {
		aic3x_init_3007(codec);
		ret=snd_soc_write(codec, CLASSD_CTRL, 0);
		if(ret<0)
			return ret;
	}
...
static int aic3x_probe(struct snd_soc_codec *codec)
{
	...
	ret=snd_soc_write(codec, MICBIAS_CTRL, 0xc0);
    if(ret<0)
		return ret;
	if (gpio_is_valid(aic3x->gpio_reset) &&
